...
class Ciudades extends CI_Controller
{
    ...
    public function obtener_listado_ciudades()
    {
        ...
        if (validarUsuario(false)) {
            $datos = $this->ciudades_model->obtener_listado_ciudades();
    ...
    public function agregar_ciudades()
    {
        $mensaje = new stdClass();
        $this->load->helper('array_utf8');
        if (validarUsuario(true)) {
            $validator = form_ciudades('agregar');
            if ($validator->respuesta == 'S') {
                $this->load->model('/administracion/ciudades_model', 'ciudades_model');
                $descripcion = $this->input->post('descripcion');
                $this->ciudades_model->ingresar_ciudades($descripcion);
                $mensaje->respuesta = 'S';
                $mensaje->data = 'ciudades Modificado Correctamente';
            } else {
                $mensaje->respuesta = 'N';
    ...
    public function editar_ciudades()
    {
        $mensaje = new stdClass();
        $this->load->helper('array_utf8');
        if (validarUsuario(true)) {
            $validator = form_ciudades('editar');
            if ($validator->respuesta == 'S') {
                $this->load->model('/administracion/ciudades_model');
                $id = $this->input->post('id');
                $descripcion = $this->input->post('descripcion');
                $this->ciudades_model->editar_ciudades($id, $descripcion);
                $mensaje->respuesta = 'S';
                $mensaje->data = 'ciudades Modificado Correctamente';
            } else {
                $mensaje->respuesta = 'N';
    ...
    public function cambiar_estado_ciudades()
    {
        $mensaje = new stdClass();
        $this->load->helper('array_utf8');
        if (validarUsuario(true)) {
            $validator = form_ciudades('estado');
            if ($validator->respuesta == 'S') {
                $this->load->model('/administracion/ciudades_model');
                $id = $this->input->post('id');
                $perfil = $this->input->post('estado');
                if ($perfil == 'S') {
                    $this->ciudades_model->cambia_estado_ciudades($id, 'N');
                    $mensaje->respuesta = 'S';
                } elseif ($perfil == 'N') {
                    $this->ciudades_model->cambia_estado_ciudades($id, 'S');
                    $mensaje->respuesta = 'S';
                } else {
                    $mensaje->respuesta = 'N';
