$(document).ready(function () {
    // Variables Globales
    var tabla = $('#tabla_estados_pedidos')
    var btn_agregar = $('#btn_agregar_estados_pedidos')
    var modal_alerta_agregar_editar = $('#modal_alerta_agregar_editar')
    var mdl_agregar_editar = $('#modal_agregar_editar_estados_pedidos')
    var mdl_titulo_agregar_editar = $('#titulo_agregar_editar_estados_pedidos')
    ...
    var table = tabla.DataTable({
        ...
        'ajax': {
            'url': '/delysoft/administracion/estados_pedidos/obtener_listado_estados_pedidos',
            'datatype': 'json',
            'dataSrc': 'data',
            'type': 'post',
        },
    ...
    btn_agregar.on('click', function () {
        ...
        mdl_titulo_agregar_editar.text('Agregar Estados Pedidos Sistema')
        mdl_agregar_editar.modal('show')
    })
    tabla.on('click', '.btn_editar', function () {
        limpieza_modal()
        mdl_btn_editar.show()
        mdl_titulo_agregar_editar.text('Editar Estados Pedidos Sistema')
    ...
    // Agregar Estados Pedidos
    mdl_btn_agregar.on('click', function () {
        var array = {
            'descripcion': mdl_descripcion.val(),
            'nombre': mdl_nombre.val()
        }
        var request = envia_ajax('/delysoft/administracion/estados_pedidos/agregar_estados_pedidos', array)
    ...
    mdl_btn_editar.on('click', function () {
        ...
        var request = envia_ajax(
            '/delysoft/administracion/estados_pedidos/editar_estados_pedidos', array)
        request.fail(function () {
            $('#modal_generico_body').html('Error al enviar peticion porfavor recargue la pagina')
            $('#modal_generico').modal('show')
        })
    ...
    tabla.on('click', '.btn_estado', function () {
        ...
        var request = envia_ajax(
            '/delysoft/administracion/estados_pedidos/cambiar_estado_estados_pedidos', array)
        request.fail(function () {
            $('#modal_generico_body').html('Error al enviar peticion porfavor recargue la pagina')
            $('#modal_generico').modal('show')
        })
